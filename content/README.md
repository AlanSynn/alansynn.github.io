...
> schema at build (`src/lib/content-schema.ts`; project/blog frontmatter is in
> `src/content.config.ts`). A typo, a wrong type, a missing required field, *or
> an unknown key* fails the build loudly with a located error — so a field can
...
- `papers.bib` — **the only publication source.** Add `@inproceedings{...}` /
  ...
  `venues.yaml` or the build fails (the badge would render bare). (`papers.json`
  is gitignored and regenerated by `just pdfs` — never hand-edit it.)
...
- `projects/*.md` — one file per project (frontmatter + markdown body). Each
  ...
  `#publications`); the default `category: work` surfaces it. The frontmatter
  schema + every optional block lives in `src/content.config.ts`;
  `example-graphics.md` (`unlisted: true`) is the full-feature copy-and-trim
  template. A *work* project also needs a static route file
